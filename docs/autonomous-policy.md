...
**Repo carve-out (chess-web, per human standing instruction 2026-06-04).** `main`'s branch protection requires a review the orchestrator cannot supply (`mergeStateStatus: BLOCKED`, `reviewDecision: REVIEW_REQUIRED`), and the human has chosen PR-based landing over direct pushes. Standing instruction: **do not halt at gate 4 for the missing review** — instead admin-merge PRs that are otherwise mechanically mergeable and carry no escalation-valve risk signal, using `gh pr merge <url> --merge --admin --delete-branch`. All other auto-merge criteria and the escalation valve still apply unchanged (verify `mergeable: MERGEABLE`, no `CHANGES_REQUESTED`, no unresolved threads, no `Confidence: low` / non-trivial-smoke risk before merging). This carve-out covers wave, smoke, reviewer, and orchestrator-bookkeeping PRs. See `docs/decisions.md` → "main requires review; autopilot admin-merges clean PRs". (Used for PRs #1, #3–#6.)
